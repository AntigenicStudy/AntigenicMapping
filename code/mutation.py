    ...
    with strategy.scope():
        if args.model_name == 'hmm':
            ...
            model = DNNLanguageModel(
                ...
                cache_dir='fluah1/{}'.format(args.namespace),
                seed=args.seed,
                verbose=True,
            )
...
def batch_train(args, model, seqs, vocabulary, batch_size=5000,
    ...
    for epoch in range(n_epochs):
        ...
        fname_prefix = ('fluah1/{0}/checkpoints/{1}/{1}_{2}'
...
def embed_seqs(args, model, seqs, vocabulary,
    ...
    if use_cache:
        mkdir_p('fluah1/{}/embedding'.format(namespace))
        embed_fname = ('fluah1/{}/embedding/{}_{}.npy'
...
def analyze_semantics(args, model, vocabulary, seq_to_mutate, escape_seqs,
    ...
    if plot_acquisition:
        dirname = ('fluah1/{}/semantics/cache'.format(args.namespace))
        mkdir_p(dirname)
...
def analyze_reinfection(
    ...
    dirname = ('fluah1/{}/reinfection/cache'.format(args.namespace))
    mkdir_p(dirname)
...
def null_combinatorial_fitness(
    ...
    dirname = ('fluah1/{}/combinatorial/cache'.format(args.namespace))
    mkdir_p(dirname)
    fname = dirname + '/{}_mut_{}.txt'.format(args.namespace,
                                              n_mutations)
