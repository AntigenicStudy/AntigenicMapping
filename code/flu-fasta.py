...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='Influenza sequence analysis')
    ...
    parser.add_argument('--checkpoint', type=str, default='models/flu.hdf5',
...
def compute_statistics(baseline_fname, target_fname,
    ...
    print("running time for embedding is ", emb_end_time - emb_start_time)

    
    mut_change = np.sum(np.abs(base_embedding.mean(0) - base_embedding.mean(0)))
    ...
    np.savetxt('matrix/h3-recon/flu_wt.txt',base_embedding,fmt='%f',delimiter=' ')
    ...
    for record in SeqIO.parse(target_fname, 'fasta'):
        ...
        np.savetxt('matrix/h3-recon/flu_matrix'+str(record_n)+'.txt',mut_embedding,fmt='%f',delimiter=' ')
...
if __name__ == '__main__':
    ...
    vocab_size = len(AAs) + 3

    model = get_model(args, 578, vocab_size,
